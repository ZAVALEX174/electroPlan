...
test("ОСИРОТЕВШАЯ группа на розетке ждёт своего часа: пропал артикул — сменился механизм соседа", () => {
  /* ЗАЧЕМ ЭТОТ ТЕСТ. Пока конструктор при замене клавиши на розетку сохранял группу (см.
     EPBuilderSlots.replaceAt), она оставалась в проекте невидимо: поля у розетки нет, человек
     её не видит и убрать не может. Пока товар розетки в каталоге — правило выше молчит, и
     дефект выглядит безобидным. Но прайс перезаливают до 7 раз в год, и как только артикул
     пропадёт, та же позиция станет ПОТЕРЯННОЙ КЛАВИШЕЙ: N группы вырастет с 2 до 3, и ЧУЖОЙ,
     нетронутый пост переедет с переключателя (25.79 €) на инвертор (42.33 €). Здесь зафиксирована
     именно эта цена ошибки — почему группу обязан уносить конструктор, а не «как-нибудь потом»
     фильтровать расчёт. */
  const twoKeys = [
    { id: "p2", number: 2, mechanismIds: [1], keyGroups: ["Кухня"] },
    { id: "p3", number: 3, mechanismIds: [1], keyGroups: ["Кухня"] }
  ];
  const clean = planOf([{ id: "p1", number: 1, mechanismIds: [999], keyGroups: [""] }].concat(twoKeys)).plan;
  const orphan = planOf([{ id: "p1", number: 1, mechanismIds: [999], keyGroups: ["Кухня"] }].concat(twoKeys)).plan;
  assert.equal(clean.groups[0].placeCount, 2, "мест в группе ровно два — клавиш две");
  assert.equal(orphan.groups[0].placeCount, 3, "осиротевшая группа добавила фантомное место");
  assert.deepEqual(clean.places.map(p => p.code), ["20005.0", "20005.0"]);
  assert.deepEqual(orphan.places.map(p => p.code), [null, "20005.0", "20013.0"],
    "нетронутый пост № 3 получил инвертор вместо переключателя");
});
