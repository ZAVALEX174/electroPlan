/* ПОВЕДЕНЧЕСКИЙ регресс E13: селектор «Коллекция накладок» в панели свойств комнаты
   ...
   МУТАЦИОННАЯ ТАБЛИЦА (проверено, см. отчёт; §3–6 — асёрты одного теста «§3–6 onchange…»):
     ...
     esc не экранирует ' (убрать ' из класса в js/app.js:40)   → красит §7-esc (value/text с сырым ', не &#39;).
...
test("§7-esc: имя коллекции из каталога ЭКРАНИРУЕТСЯ в опциях, а не вставляется сырым", () => {
  /* Название коллекции приходит из каталога (productSeries → productCollections), а прайс
     ...
     Подаём каталог из одной накладки с series=['A\'"><b>&'] и НАСТОЯЩИЙ esc; опасное имя несёт ВСЕ
     ПЯТЬ символов, которые esc обязан экранировать (& < > " '), — прежняя фикстура забывала
     апостроф, и мутация «esc не экранирует '» оставалась зелёной. Опция обязана нести
  ...
  assert.equal(opt.value, "A&#39;&quot;&gt;&lt;b&gt;&amp;", "value опции экранирован полностью, включая апостроф (&#39;)");
  assert.equal(opt.text, "A&#39;&quot;&gt;&lt;b&gt;&amp;", "видимый текст опции экранирован полностью, включая апостроф (&#39;)");
