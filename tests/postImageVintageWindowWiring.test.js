/* РЕГРЕСС ВИНТАЖНОГО ПОСТА (владелец, 16.09 → задача НАКЛАДКА-ФОТО).
   ...
   у 22674.4 честно ЧЕТЫРЕ окна, как у эталонной 22684 «Flat 4 на 4». Число окон (4) совпало с числом
   одномодульных ЯЧЕЕК (4), поэтому postWindows раскладывает ПОМОДУЛЬНО (окно i — отверстие модуля i),
   а windowsHoldModules подтверждает: каждое узкое окно физически держит свой ОДИН модуль (ratio ≈0,6
   ≥ 0,375) — пост уходит в РЕЖИМ ФОТО с фоновым снимком накладки, и наружу через отверстия торчат
   только переключатели. Первый тест это фиксирует (упал бы при откате данных к одному окну). Второй
   тест сохраняет покрытие САМОЙ защиты windowsHoldModules на синтетике — единственное недостоверно
   узкое окно на ВЕСЬ пост из 4 модулей она обязана отклонять и после починки данных.
...
test("винтажный пост 22674: четыре измеренных окна → режим фото (не схема)", async () => {
  ...
  /* 4 окна = 4 одномодульным ячейкам → помодульная раскладка; каждое окно держит свой ОДИН модуль. */
  const pw = EPPostImage.postWindows(spec.frame.opening, spec.rows, spec.frame.windows);
  assert.equal(pw.perModule, true, "окно на каждый модуль (число окон совпало с числом модулей, не постов)");
  assert.equal(EPPostImage.windowsHoldModules(spec.frame.opening, spec.rows, spec.frame.windows), true,
    "каждое узкое окно физически держит свой один модуль (перекос из полоски на 4 модуля не возникает)");
