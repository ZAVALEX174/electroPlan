...
test("НЕЧИСЛОВОЙ счётчик даёт прочерк, а не «null мест» в интерфейсе и документах", () => {
  /* Счётчик подставлялся в строку сырым, а склонение при нечисловом входе честно давало
     множественную форму — вместе это печатало «null мест» и «undefined модулей» человеку. */
  [null, undefined, NaN, Infinity, "", "  ", "много", {}, []].forEach(bad => {
    assert.equal(placeWord(bad), "— мест", `placeWord(${String(bad)})`);
    assert.equal(moduleWord(bad), "— модулей", `moduleWord(${String(bad)})`);
  });
});
test("числовая строка считается числом — подпись не теряет счётчик из атрибута разметки", () => {
  assert.equal(placeWord("1"), "1 место");
  assert.equal(moduleWord(" 3 "), "3 модуля");
});
...
test("productsForRoom: сравнение коллекции РЕГИСТРОЗАВИСИМО — 'arke' не подменяет 'Arke'", () => {
  /* ЗАДУМАННОЕ поведение (js/catalog.js): collection всегда приходит из productCollections, регистр
     ...
     compatibleMechanisms). Иной регистр в room.collection (старый/правленый проект) до сюда НЕ
     доходит: EPRoom.roomCollection валидирует значение по списку каталога и отбраковывает мусор
     любого вида в null («коллекция не задана») ещё раньше — на null criteria.collection пуст,
     фильтра нет выше по строке. Поэтому пустого результата и деградации через фолбэк
     collectionFramePool на текущем коде НЕ возникает (ветка pool.length?pool:allFrames не
     исполняется). Регистрозависимость productsForRoom оставлена НАМЕРЕННО — но её роль иная:
     страховка от тихой подмены состава на случай, если источник списка когда-нибудь разойдётся с
