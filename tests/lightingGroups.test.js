...
test("контракт findMechanism: роль, серия и ВИД места (kind+source) — и только они", () => {
  /* Контракт расширен видом места (клавиша/цельное): цельному изделию нужна замена САМОГО артикула,
     клавише — голый механизм, и один вопрос «роль+серия» их не различает. Но всё, что едет в
     аргумент, ОБЯЗАНО ехать и в ключ кэша (иначе кэш отдал бы всем ответ первого спросившего), —
     поэтому в аргументе ровно {role, series, kind, source} и ничего сверх (ни место, ни группа). */
  const seen = [];
  ...
  seen.forEach(keys => assert.deepEqual(keys, ["kind", "role", "series", "source"]));
});
