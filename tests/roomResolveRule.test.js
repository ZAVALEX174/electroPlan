...
/* --- НЕОДНОЗНАЧНОСТЬ grid-ветви: несколько подписей в ОДНОЙ открытой компоненте ------------------
   Мотивирующий дефект владельца («пост ушёл не туда»): инструмент «Комната» ставит подписи без
   контура; без стен между ними componentAt даёт всем ОДИН componentId, и прежний
   find(r=>r.componentId===component) возвращал ПЕРВУЮ по списку — объект молча уходил в чужую
   комнату (чужая отделка, чужой раздел сметы). Ветвь обязана разрешать это свидетельством —
   ближайшим достижимым ЯКОРЁМ, а не порядком в массиве. Компонента общая → wholePlaneMap. */

test("две подписи в одной компоненте без стен: объект уходит к БЛИЖАЙШЕЙ, а не к первой в списке", () => {
  const ctx = {
    polyRooms: [], walls: [], map: wholePlaneMap(7),
    /* «Первая» специально стоит первой в массиве и ДАЛЕКО — прежний find вернул бы её. */
    gridRooms: [
      { id: "Первая", componentId: 7, seedX: 1000, seedY: 500 },
      { id: "Ближняя", componentId: 7, seedX: 100, seedY: 100 }
    ]
  };
  assert.equal(resolve(120, 120, ctx)?.id, "Ближняя",
    "объект рядом с якорем «Ближняя» не должен приписываться первой подписи списка");
});

test("перестановка комнат в списке НЕ меняет ответ grid-ветви (сегодня меняла — это и есть дефект)", () => {
  const far = { id: "far", componentId: 7, seedX: 1000, seedY: 500 };
  const near = { id: "near", componentId: 7, seedX: 100, seedY: 100 };
  const makeCtx = gridRooms => ({ polyRooms: [], walls: [], map: wholePlaneMap(7), gridRooms });
  assert.equal(resolve(120, 120, makeCtx([far, near]))?.id, "near");
  assert.equal(resolve(120, 120, makeCtx([near, far]))?.id, "near",
    "ближайшая подпись выбирается по расстоянию до якоря, а не по позиции в state.rooms");
});

test("стена между точкой и ближайшим якорем: берётся достижимый якорь, а не перекрытый", () => {
  const ctx = {
    polyRooms: [], map: wholePlaneMap(7),
    /* Глухая вертикальная стена x=150 отсекает точку (200,200) от близкого якоря «Near» (100,200),
       но не от дальнего «Far» (400,200): объект не тянем сквозь стену. */
    walls: [wall(150, 100, 150, 300)],
    gridRooms: [
      { id: "Near", componentId: 7, seedX: 100, seedY: 200 },
      { id: "Far", componentId: 7, seedX: 400, seedY: 200 }
    ]
  };
  assert.equal(resolve(200, 200, ctx)?.id, "Far",
    "ближайший якорь перекрыт стеной — берётся дальний достижимый, а не перекрытый ближний");
});

test("ровно посередине между двумя якорями: честный null (объект «вне помещений»)", () => {
  const ctx = {
    polyRooms: [], walls: [], map: wholePlaneMap(7),
    gridRooms: [
      { id: "L", componentId: 7, seedX: 100, seedY: 200 },
      { id: "R", componentId: 7, seedX: 300, seedY: 200 }
    ]
  };
  assert.equal(resolve(200, 200, ctx), null,
    "равноудалённые якоря не выбираются по порядку/id — объект остаётся видимой сиротой");
});

test("контур сильнее подписи: точка внутри полигональной комнаты не перехватывается grid-подписью", () => {
  const ctx = {
    /* Подпись «Метка» в той же компоненте, её якорь ровно в точке — но точка ВНУТРИ контура,
       поэтому ветвь попадания решает первой и grid-развилка не срабатывает вовсе. */
    polyRooms: [{ id: "Контур", polygon: rect(0, 0, 200, 200) }],
    gridRooms: [{ id: "Метка", componentId: 7, seedX: 100, seedY: 100 }],
    map: wholePlaneMap(7), walls: []
  };
  assert.equal(resolve(100, 100, ctx)?.id, "Контур",
    "настоящее попадание в контур сильнее любой подписи, даже с якорем в самой точке");
});
