/* Структурный регресс-тест ЕДИНСТВЕННОСТИ правила «в какой комнате точка» в js/app.js.
   ...
   pointInPolygon, захардкодить число вместо EPConfig.roomEdgeTolerance, перенести построение
   контекста внутрь цикла — каждая мутация обязана ронять соответствующий assert. */
...
test("getRoomForPoint делегирует в resolveRoomForPoint и не держит своей копии правила", () => {
  const body = functionBody("getRoomForPoint");
  assert.ok(
...
test("recalculateRoomAssignments делегирует в resolveRoomForPoint и не держит своей копии правила", () => {
  const body = functionBody("recalculateRoomAssignments");
  assert.ok(
    /resolveRoomForPoint\s*\(/.test(body),
...
test("resolveRoomForPoint берёт допуск из EPConfig, а не хардкодит число", () => {
  const body = functionBody("resolveRoomForPoint");
  assert.ok(
    /EPRoomAssign\s*\.\s*nearestRoomWithinTolerance\s*\(/.test(body),
    "допуск у границы обязан считаться через EPRoomAssign.nearestRoomWithinTolerance"
  );
  assert.ok(
    /EPConfig\s*\.\s*roomEdgeTolerance/.test(body),
    "величина допуска обязана приходить из EPConfig.roomEdgeTolerance, а не быть числом в коде"
  );
});
