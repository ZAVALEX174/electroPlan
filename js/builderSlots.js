...
/* replaceAt(slots, index, id, isKey?) — заменить механизм слота, сохранив его группу света.
   ...
   isKey(id) → bool: спрашиваем приложение (каталог знает только оно), клавиша ли НОВЫЙ
   механизм. Предиката нет — модуль о каталоге не знает и ведёт себя как прежде (группа
   сохраняется); приложение обязано его передавать. */
function replaceAt(slots, index, id, isKey) {
  const list = Array.isArray(slots) ? slots : [];
  const i = Number(index);
  const keepGroup = typeof isKey === "function" ? !!isKey(id) : true;
