...
function planByRooms(input) {
  ...
  /* Пустой проект — ОДИН расчёт по схеме проекта, точно как раньше. Форма плана уже нужная,
     сливать нечего, а пробел неподдержанной/нераспознанной схемы на пустом проекте обязан
     сохраниться: его plan ставит и при нуле мест (интерфейсу нужна причина, даже когда постов
     нет). Штатные схемы, включая «Звонковые кнопки», поддержаны — на пустом проекте у них пробела
     схемы нет. */
  ...
  ordered.forEach(bucket => {
    ...
    /* supported СЛИВАЕМ через ИЛИ: план поддержан, если ПО КРАЙНЕЙ МЕРЕ ОДНА партиция посчитана по
       поддержанной схеме. Прежнее И лгало на смешанном проекте, где одна комната по НЕПОДДЕРЖАННОЙ
       схеме (supported=false): оно объявляло весь план supported=false, одновременно возвращая
       подобранные механизмы в totals — план утверждал о себе неправду. С ИЛИ противоречия нет:
       totals непусты ⇒ supported=true; supported=false остаётся только когда НИ ОДНА партиция не
       поддержана — тогда totals пусты, и это правда. Непосчитанные партиции не теряются молча:
       каждая сигналит о себе своим пробелом scheme-not-implemented и missingReason у мест — их несёт
       merged.gaps. Флаг проекта же честно говорит лишь «есть ли в плане хоть что-то посчитанное», а
       не «всё ли посчитано». (Все три штатные схемы, включая «Звонковые кнопки», сейчас поддержаны;
       ветка ждёт будущую схему без правил расчёта.) */
...
/* Подпись кэша расчёта — тоже ЗДЕСЬ и под тестом: без комнат в подписи смена схемы у одной
   ...
   группы, НОМЕРА ПРОХОДНЫХ и РУЧНОЙ ВЫБОР МЕХАНИЗМА клавиш, номер поста (решает канонический
   порядок ролей) И ПРИВЯЗКА К КОМНАТЕ (roomId), по каждой комнате — её ДЕЙСТВУЮЩАЯ схема
   (schemeOf, обычно EPRoom.roomLightingScheme). Смена схемы у комнаты меняет её запись, переезд
   поста — его roomId; и то и другое сбрасывает кэш.
   ⚠️ keyCrossNumbers и keyMechanisms ОБЯЗАНЫ быть здесь наравне с keyGroups: расчёт читает их
   тем же keyIndex (EPLightingPlan.collect → place.crossNo/roleOverride), и клавиша получает по ним
   переключатель вместо выключателя. Без них проставленный «№ проходной» (два поста склеиваются в
   одну проходную) и ручная замена механизма меняли план, а подпись — нет: кэш отдавал старые
   отдельные группы, и блок «Группы света» с суммами оживал лишь после постороннего действия
   (переключения схемы), которое подпись всё-таки сдвигало. Один пропуск — сразу два молча
   застывающих пути (номер проходной и ручной механизм), поэтому поля добавлены оба. */
function cacheSignature(input) {
  ...
  return JSON.stringify([
    o.projectScheme,
    o.productCount,
    posts.map(p => [p.id, p.number, p.mechanismIds, p.keyGroups, p.keyCrossNumbers, p.keyMechanisms, p.roomId == null ? null : p.roomId]),
