...
window.EP_DATA = {
  settings: {
    ...
    wallType: "solid",
    // Схема электрики проекта: classic — «Классическая» (выключатель/переключатели/инверторы
    // по числу мест управления группой), relay — «Реле» (кнопки + импульсное реле на группу),
    // bell — «Звонковые кнопки» (кнопка на каждом месте, как в «Реле», но импульсные реле не считаются).
    // Свойство ПРОЕКТА, как и тип стены: определяет, какой механизм подставляется за клавишу.
    // Проекты, сохранённые до появления поля, открываются с классической — это и требуется.
    lightingScheme: "classic",
    // Подсветка клавиш — свойство ПРОЕКТА (решение владельца 09.09): считать её вообще
    // (enabled), каким цветом (color) и на какое напряжение (voltage). По умолчанию
    // ВЫКЛЮЧЕНА — включение добавляет в смету аксессуары-LED, и старые проекты без этого
    // поля обязаны открываться без наценки (см. restoreProject, тот же принцип, что у
    // надбавки к курсу). Цвет по умолчанию — «Белая»: нейтральный индикатор, при выключенной
    // подсветке значения не касается вовсе. Напряжение по умолчанию 110-250V — распространённая
    // сетевая подсветка (переопределение у конкретного поста — отдельная задача B2). Цвет и
    // напряжение матчатся строго (===), поэтому список берётся из каталога, а не хардкодится.
    backlight: { enabled: false, color: "Белая", voltage: "110-250V" },
    // Вид поста в листе монтажника (Б2): "exploded" — взрыв-схема (по деталям, умолчание),
    // "assembled" — собранная накладка с клавишами. Свойство ПРОЕКТА, как тип стены и схема
    // электрики, а НЕ часть набора столбцов КП (наборы его не читают и не переписывают).
    // Старые проекты без поля открываются со взрыв-схемой — прежнее поведение (см. restoreProject).
  ...
  products: (function enrichCatalog(){
    ...
    /* Функциональная группа и подгруппа механизма («управление светом», «Розетки»,
       «Информационные разъемы»…) — РАЗДЕЛЫ ВЫБОРА товара в полноэкранном конструкторе поста.
       Берём их из номенклатуры заказчика (раздел groups), а не из categoryId: тот ставит
       эвристика classify() по названию, и её разделы расходятся с теми, которыми думает
    ...
    return list.map(p => {
      ...
      /* Раздел выбора товара: functionalGroup — крупный раздел («управление светом»),
         functionalSubgroup — второй уровень («Выключатели», «Диммеры»), заполнен у 161
         механизма из 435. Полей нет, если номенклатура признака не даёт, — «раздела нет»
         отличается от «раздел пустой» по отсутствию ключа, как у principle/partRole. */
