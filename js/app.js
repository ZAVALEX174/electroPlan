...
/* ⚠️ ЕДИНОЕ ПРАВИЛО «В КАКОЙ КОМНАТЕ ТОЧКА». Раньше оно жило в ДВУХ местах (getRoomForPoint для
   ...
   Свидетельства — от сильнейшего к слабейшему, но grid НЕ конкурирует с допуском в очереди: он
   ПРОВЕРЯЕТ кандидата от допуска. Причина — компонента связности через дверные проёмы накрывает
   всю квартиру, поэтому «grid слабее допуска» верно лишь когда компонента объединяет несколько
   комнат; когда полигональная комната замкнута, а компонента точки ДРУГАЯ, компонента — прямое
   доказательство, что объект за стеной и к этому контуру не относится. Ветви:
     1) настоящее попадание в КОНТУР комнаты (pointInPolygon) — прямое доказательство «точка внутри»;
        решает сразу и не платит за проверки ниже;
     2) КАНДИДАТ от допуска у границы контура (EPRoomAssign): выключатель у дверного проёма стоит
        центром ровно на линии, pointInPolygon считает это «снаружи». Кандидата сверяем компонентой:
          · карты нет (grid-комнат в проекте нет) — противоречить нечему, кандидат принят;
          · компонента ТОЧКИ и компонента SEED кандидата совпали — контур по ту же сторону стены,
            кандидат принят;
          · разошлись — между ними стена, кандидат ОТВЕРГНУТ, идём к grid.
        Допуск ищет ближайший КОНТУР, grid-комнаты в нём не участвуют — у них контура нет;
     3) GRID-комната по компоненту связности — для комнат без контура (ручная подпись без полигона).
   Компоненту SEED кандидата берём по его seedX/seedY (у полигональных комнат это центроид контура,
   проставленный при создании); если seed почему-то отсутствует — считаем центроид тем же
   polygonCentroid, второго способа не заводим. Проверка компоненты выполняется ТОЛЬКО когда допуск
   что-то нашёл (объект вне комнат) — объект, попавший в контур, за неё не платит. */
function resolveRoomForPoint(cx,cy,ctx){
  ...
  if(near){
    if(!ctx.map)return near;   /* grid-карты нет — компоненте нечего опровергать, кандидат принят */
    const seed=near.seedX!=null?{x:near.seedX,y:near.seedY}:polygonCentroid(near.polygon);
    /* совпали компоненты точки и seed кандидата — контур по ту же сторону стены; разошлись — стена
       между ними, близость к контуру обманчива, кандидата отвергаем и уходим в grid-ветвь */
    if(componentAt(ctx.map,cx,cy)===componentAt(ctx.map,seed.x,seed.y))return near;
  }
