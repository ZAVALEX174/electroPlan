...
/* Лицо из bbox выбранного компонента и span. Высота лица выводится из ширины компонента по
   пропорции модуля, по вертикали центрируется (монтажные лапки сверху/снизу отсекаются) — КРОМЕ
   заметно ВЫСОКИХ компонентов (розетка с откинутой крышкой): там лицо прижимается к нижней границе,
   см. ниже. Затем поджимается на TRIM и переводится в % фото. clamped — расчётное лицо оказалось
   выше компонента и урезано до него. */
export function faceRectFromComponent(comp, span, w, h) {
  ...
  /* Обычно лицо ЦЕНТРИРУЕМ по вертикали — монтажные лапки сверху и снизу симметричны, центр их
     срезает. НО у розеток с ОТКИДНОЙ КРЫШКОЙ (09208.C, 14208.C, 20208.C…) открытая крышка торчит
     ВВЕРХ над корпусом, а сам модуль сидит в НИЖНЕЙ части кадра: компонент выходит заметно ВЫШЕ
     ожидаемого (ratio < 1−ASPECT_TOL, т.е. заметно у́же/выше пропорции модуля). Центрирование тогда
     захватило бы крышку сверху и срезало низ модуля — прижимаем лицо к НИЖНЕЙ границе компонента.
     Если компонент, наоборот, ШИРЕ ожидаемого (ratio высокий) — это не крышка, оставляем центр. */
  let faceTop = ratio < 1 - ASPECT_TOL
    ? comp.top + (bboxH - faceH)          // прижать к низу компонента (крышка сверху)
    : comp.top + (bboxH - faceH) / 2;     // центрировать
  let clamped = false;
