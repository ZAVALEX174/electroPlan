...
async function main() {
  ...
  for (const rec of records) {
    ...
    /* В файл кладём ТОЛЬКО поля, которые читает рантайм (js/data.js подмешивает признаки
       ...
       По тому же правилу туда ушёл и раздел roles — роль детали в посте («голый механизм» /
       «клавиша») и роль управления из колонок «Функциональная группа» + «Тип управления».
       Раньше рантайм различал их косвенно: по categoryId=900 (эвристика classify() по слову
       «клавиш» в названии) и по отсутствию moduleSpan. Признак есть у 169 позиций из 2146 —
       в горячем каталоге это были бы 2146 лишних ключей ради 8% строк. */
    const product = {
      id,
